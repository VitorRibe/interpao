...
class GetProgressoEquipeUseCase:
    ...
    async def execute(self) -> list[ProgressoFuncionario]:
        # 1. Procura todas as trilhas, seu setor associado (nome e id) e calcula o total de módulos
        query_trilhas = text("""
            SELECT t.id_trilha AS trilha_id, t.titulo, t.id_setor, s.nome AS setor_nome, COUNT(m.id_modulo) AS total_modulos
            FROM trilha t
            LEFT JOIN setor s ON t.id_setor = s.id_setor
            LEFT JOIN modulo m ON m.id_trilha = t.id_trilha
            GROUP BY t.id_trilha, t.titulo, t.id_setor, s.nome
        """)
        ...
        for u in users_rows:
            ...
            for tid, t_data in trilhas_dict.items():
                ...
                trilha_nome_setor = (t_data["setor_nome"] or "").strip().lower()
                
                # REGRA DE NEGÓCIO DA INTERPÃO:
                # A trilha é obrigatória se for do mesmo setor do funcionário,
                # OU se for uma trilha "Geral" (id_setor nulo ou setor com nome 'geral').
                is_geral = trilha_id_setor_str is None or trilha_nome_setor == "geral"
                is_mesmo_setor = trilha_id_setor_str == user_id_setor_str
                
                if not (is_geral or is_mesmo_setor):
                    continue  # Ignora a trilha pois não pertence ao funcionário nem é geral
